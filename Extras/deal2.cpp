...
double FEM<dim>::basis_function(unsigned int node, double xi){
 ...
 // Given Xi and the node , I need to specify the value of X at the given xi and the node
  //EDIT
...
double FEM<dim>::basis_gradient(unsigned int node, double xi){
  ...
  // Reference :  https://math.stackexchange.com/questions/809927/first-derivative-of-lagrange-polynomial
...
void FEM<dim>::setup_system(){
  ...
  // THIVIN - Choose quadrature points based on the value of the order of basis function used.
  // i.e , Using cubic polynomial will have order of 4 in Stiffness matrix ( Derivative * derivative ) , So to approximate 4th order polynomial we need 3 Quadrature points
/*  
...
quad_weight[0] = 0.6521451548625461; //EDIT
quad_weight[1] = 0.6521451548625461; //EDIT
...
//EDIT - Define Flocal.
// THIVIN - F local = f * A * h_e / 2
Flocal[A] += f * Area *h_e*0.5* basis_function(A,quad_points[q]) * quad_weight[q] * x;
      }
    }
...
for(unsigned int q=0; q<quadRule; q++){
  //EDIT - Define Klocal.
   // 2 * E * A / he
    Klocal[A][B] += ((E * Area * 2)/ h_e)  * quad_weight[q] * basis_gradient(A,quad_points[q]) * basis_gradient(B,quad_points[q]);  
    //std::cout << " A : " <<A << "  B: " << B << "  Kloc : " << Klocal[A][B] << std::endl;
}
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++){
      ...
      //THIVIN - Ignoring the intitial and the final node boundary condition , just assemble the complete matrix
...
u_exact = 0.;
if (prob == 1)
  u_exact =  -( ( x * x * x )/6 ) + ( ((0.07)/6)*x) ;
if (prob == 2)
  u_exact =  -( ( x * x * x )/6 ) + ((0.105)*x);
l2norm += quad_weight[q] * ( u_h - u_exact) * ( u_h - u_exact) * h_e * 0.5;
